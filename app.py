    ...
    if request.method == 'POST':
        class SLR:
            def __init__(self):
                ...
                self.RR = []
                self.RR = []
            file_name = request.form['grammar']
            def _read_grammar(self):
                try:
                    with open(s.file_name) as grammar_file:
                                            ...
                                            if current_variable in self.non_terminals:
                                                for each_grammar in self.new_grammar:
                                                    if each_grammar[0] == current_variable and each_grammar[
                                                        1] != "'" and each_grammar not in added_grammar:
                                                        added_grammar.append(each_grammar)
        ...
        return render_template('string.html',s=s,grammar=s.file_name)
...
def result():
    if request.method == 'POST':
        class SLR:
            ...
            print("\nTest String: ", string)
        s = SLR()
        result = s.test_simulation(s.string)
    if result == 1:
        return render_template('result.html',result="Accepted")
    else:
        return render_template('result.html', result="Not Accepted")
